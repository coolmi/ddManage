...
//    },
    created() {
      ...
      _that.flowParams = this.$route.query.flowParams;
      if (info === 'next') {
        flowRU.getDBList(function (res) {
          if (res.page.count !== 0) {
            _that.flowParams = flowGLU.getList(res.page.list[res.page.count - 1], 'db');
            _that.getRouteHandleInfo(_that.flowParams);
          }
        })
      } else {
        _that.getRouteHandleInfo(_that.flowParams);
